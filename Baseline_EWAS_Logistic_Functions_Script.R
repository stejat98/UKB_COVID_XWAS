## EWAS Functions (poisson (log link) regression)

## code inspired by "chiragjp/xwas" and "chiragjp/dhs_india/xwas"
...
constructFormulaLogistic <- function (exposure, data, depvar, adjustments) {
  ...
  if (is.null(adjustments)) {
    formula <- sprintf("%s ~ %s", depvar, exposure)
  }
  else{
    formula <- sprintf("%s ~ %s + %s", depvar, exposure, paste(adjustments, collapse = ' + '))
  }
